...
func TestTrainingRoutesAreWiredAndProtected(t *testing.T) {
	...
	a, err := New(config.Config{DatabaseURL: dsn, HTTPAddress: "127.0.0.1:0", JWTSecret: "training-integration-test-secret", JWTIssuer: "test", AccessTokenTTL: time.Minute, RefreshTokenTTL: time.Hour})
	if err != nil {
		t.Fatal(err)
	}
	defer a.Close()

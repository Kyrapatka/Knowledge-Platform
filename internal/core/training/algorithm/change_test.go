...
func TestChangeKeepsElapsedTimeAndHistory(t *testing.T) {
	...
	if next.Stage != 5 || !next.StageReviewAt.Equal(anchor.AddDate(0, 0, 7)) || next.CorrectCount != 17 || next.WrongCount != 4 || next.ConsecutiveCorrect != 0 || next.RehabActive {
		t.Fatal(next)
	}
	...
	if err != nil || next.Stage != 8 || !next.StageReviewAt.Equal(anchor.AddDate(0, 0, 46)) {
		t.Fatal(next, err)
	}
...
func TestChangeFiniteHorizonAndTrackBoundaries(t *testing.T) {
	...
	if err != nil || next.TargetAt.Sub(p.LearningStartedAt) != 180*24*time.Hour || !next.StageReviewAt.Equal(anchor.AddDate(0, 0, 35)) {
		t.Fatal(next, err)
	}
